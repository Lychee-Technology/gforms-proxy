...
> **Amended 2026-09-02 by [ADR 0009](0009-turnstile-per-form-opt-in.md):** the overwrite bullet above originally described the Turnstile regeneration guard in full (`turnstileEnabled: true` plus `--turnstile` / `--force`). That description moved to ADR 0009, which records the guard as one of the Turnstile decisions; the bullet was shortened to the cross-reference it carries now. The decision recorded here is unchanged.
