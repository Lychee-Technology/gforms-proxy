...
- Branch previews are public `workers.dev` URLs (`<branch>-gforms-proxy.iceboundrock.workers.dev`, plus one per commit) that serve the branch's bundled schemas. They are the same Worker with different code, so they share its secrets, including `TURNSTILE_SECRET_KEY`, and they carry the same `submissionUrl`: a preview accepts real submissions to the real Google Form, Turnstile and all. Use a preview to check a schema with `GET /api/v1/forms/:formId/schema` or a validation change by sending `POST /api/v1/forms/:formId/responses` bodies that fail validation (those never reach Google). Do not send real submissions through it, and do not hand the URL to a site. Previews stay live after the PR is merged.
