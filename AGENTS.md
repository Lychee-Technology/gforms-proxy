...
- A PR that changes a registered form's schema (regenerating a form, renaming keys, changing a type) ships that change to production the moment it merges. The "deploy decision" for such a change is the merge decision, and the PR rule below that merges happen only on explicit request is what gates it. Whether such changes also need to be announced before merge is an open question (#48), not a rule this section sets.
- Branch previews are public `workers.dev` URLs (`<branch>-gforms-proxy.iceboundrock.workers.dev`, plus one per commit) that serve the branch's bundled schemas. Take the exact URL from the `cloudflare-workers-and-pages` PR comment: an alias may only contain lowercase letters, digits and dashes, so a branch name with other characters, such as `/`, does not map onto it verbatim. They are the same Worker with different code, so they share its secrets, including `TURNSTILE_SECRET_KEY`, and they carry the same `submissionUrl`: a preview accepts real submissions to the real Google Form, Turnstile and all. Use a preview to check a schema with `GET /api/v1/forms/:formId/schema` or a validation change by sending `POST /api/v1/forms/:formId/responses` bodies that fail validation (those never reach Google). Do not send real submissions through it, and do not hand the URL to a site. Previews stay live after the PR is merged.
